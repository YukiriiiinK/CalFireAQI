...
function sliderFactory(d) {
  ...
  var parseNum = function idx_to_ym(i) {
      var m = i % 12 + 1;
      var y = 2009 + parseInt(i / 12);
      var ym = String(y * 100 + m);
      return ym.slice(0, 4) + '-' + ym.slice(4);
  };

  var idxToYear = function idx_to_y(i) {
    var y = 2009 + parseInt(i / 12);
    return y;
  };
  ...
  function drawSlider(p) {
      ...
      if (scale) {
        ...
        let axis=track.append("g")
          ...
          .call(d3[axisType](sliderScale)
            .tickSize(8)
            .tickValues(calculateTicks(range,ticks)).tickFormat(idxToYear)
            );
      }
    ...
    function reposition() {
      ...
      if (scale) {
        ...
        if (label) {
            let labelText=svg.selectAll(".labelText")
                .text(parseNum(rounded))
        }
    ...
    function getMousePos(orient) {
      return{
        "horizontal":sliderScale.invert(d3.event.x-43),
